...
host_target_triple() {
  ...
  case "${host_os}/${host_arch}" in
    ...
    *)
      printf 'Native frpc sidecar currently supports Darwin arm64 and x86_64 only.\n' >&2
...
# The source archive and its SHA-256 are intentionally pinned. To update frpc,
...
case "${frp_version}/${target_triple}" in
  ...
  0.71.0/x86_64-apple-darwin)
    ...
    ;;
  *)
    printf 'No audited official frpc checksum is configured for version=%s target=%s.\n' \
...
archive_member="frp_${frp_version}_darwin_${frp_arch}/frpc"
destination="${binaries_dir}/frpc-${target_triple}"
archive_path="$(mktemp "${TMPDIR:-/tmp}/frpc-${target_triple}.archive.XXXXXX")"
binary_path="$(mktemp "${binaries_dir}/.frpc-${target_triple}.extract.XXXXXX")"
...
printf 'Downloading official frpc %s for %s...\n' "${frp_version}" "${target_triple}"
curl --fail --location --proto '=https' --tlsv1.2 --retry 3 --retry-delay 1 \
  --output "${archive_path}" "${archive_url}"

actual_sha256="$(shasum -a 256 "${archive_path}" | awk '{print $1}')"
if [[ "${actual_sha256}" != "${expected_sha256}" ]]; then
  printf 'Official frpc archive checksum mismatch.\nExpected: %s\nActual:   %s\n' \
...
if [[ ! -s "${binary_path}" ]]; then
  printf 'Extracted frpc sidecar is empty.\n' >&2
  exit 1
fi
...
printf 'Installed verified official frpc sidecar: %s\n' "${destination}"
